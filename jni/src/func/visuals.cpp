...
void visuals::draw() {
    bool any = esp::GetEspConfig().box || esp::GetEspConfig().name || esp::GetEspConfig().health || esp::GetEspConfig().distance;
    if (!any) return;
    ...
    for (int i = 0; i < PlayerCount; i++) {
        ...
        if (esp::GetEspConfig().box) {
            dbox(BoxMin, BoxMax, 1.f);
        }

        if (esp::GetEspConfig().health) {
            dhp(Health, BoxMin, BoxMax, bh, 1.f);
        }

        if (esp::GetEspConfig().name) {
...
void visuals::dbox(const ImVec2& t, const ImVec2& b, float a) {
    ...
    float r = esp::GetEspConfig().box_rounding;

    ImU32 col = IM_COL32(
        static_cast<int>(esp::GetEspConfig().box_col.x * 255),
        static_cast<int>(esp::GetEspConfig().box_col.y * 255),
        static_cast<int>(esp::GetEspConfig().box_col.z * 255),
        static_cast<int>(esp::GetEspConfig().box_col.w * 255 * a)
    );

    if (esp::GetEspConfig().box_type == 0) {
...
void visuals::dhp(int hp, const ImVec2& t, const ImVec2& b, float box_h, float a) {
    ...
    ImU32 col_top = IM_COL32(
        static_cast<int>(esp::GetEspConfig().health_col.x * 255),
        static_cast<int>(esp::GetEspConfig().health_col.y * 255),
        static_cast<int>(esp::GetEspConfig().health_col.z * 255),
        static_cast<int>(esp::GetEspConfig().health_col.w * 255 * a)
    );
    ImU32 col_bot = IM_COL32(
        static_cast<int>(esp::GetEspConfig().health_col.x * 180),
        static_cast<int>(esp::GetEspConfig().health_col.y * 180),
        static_cast<int>(esp::GetEspConfig().health_col.z * 180),
        static_cast<int>(esp::GetEspConfig().health_col.w * 255 * a)
    );
...
void visuals::dnick(const char* name, const ImVec2& box_min, float cx, float sz, float a) {
    ...
    ImU32 col = IM_COL32(
        static_cast<int>(esp::GetEspConfig().name_col.x * 255),
        static_cast<int>(esp::GetEspConfig().name_col.y * 255),
        static_cast<int>(esp::GetEspConfig().name_col.z * 255),
        static_cast<int>(esp::GetEspConfig().name_col.w * 255 * a)
    );
...
void visuals::ddist(float dist, float x, float y, float sz, float a) {
    ...
    ImU32 col = IM_COL32(
        static_cast<int>(esp::GetEspConfig().distance_col.x * 255),
        static_cast<int>(esp::GetEspConfig().distance_col.y * 255),
        static_cast<int>(esp::GetEspConfig().distance_col.z * 255),
        static_cast<int>(esp::GetEspConfig().distance_col.w * 255 * a)
    );
